...
import { generateCharacter } from '../../services/generatorService';
...
const CharacterGenerator = ({ onCharacterGenerated, character }) => {
  ...
  const handleGenerateCharacter = async () => {
    ...
    try {
      ...
      // Build the prompt message with detailed instructions for the AI
      const message = `
...
Return the result as a valid JSON object with this structure:
{
  ...
  "equipment": {
    ...
    "hat": string,        // key of the selected hat or empty string
    "accessory": string,  // key of the selected accessory or empty string
    "bag": string         // key of the selected bag or empty string
  },
...
The response should be ONLY the JSON object with no additional text or explanation.
      ...
      // Log the message and context data for debugging purposes
      console.debug('AI Generation Request - Message:', message);
      console.debug('AI Generation Request - Context Data:', contextData);

      // Call the service to generate the character
      const characterData = await generateCharacter(message, contextData);
      console.debug('AI Generation Response - characterData:', characterData);

      // Validate the response format (ensure required keys exist)
      if (!characterData || !characterData.name || !characterData.appearance) {
        throw new Error('Invalid character data received from the generation service.');
      }

      // Create a base character and merge the generated data into it
      ...
      // Log the error for debugging (this will show up in your browser console)
